...
load_year = 2019
base_output_dir = "MERHourlySimulations_fixstor"
...
function convert_storage_to_loads(modified_sys, storage_timeseries, hour)
    """Convert storage units to loads (net discharge = discharge - charge)."""
    
    println("Converting storage to loads for hour $hour system...")
    
    
    storage_units = get_components(EnergyReservoirStorage, modified_sys)
    ...
    for storage in storage_units
        ...
        if charge_ts !== nothing && discharge_ts !== nothing
            ...
            PSY.add_component!(modified_sys, new_load)
    ...
    for storage in storage_to_remove
        PSY.remove_component!(modified_sys, storage)
    ...
    PSY.to_json(modified_sys, joinpath("MERsystems", modified_filename), force=true)
...
function main()
    ...
    modified_sys = convert_generators_to_loads(system_file, generator_timeseries, hour)
    modified_filename = convert_storage_to_loads(modified_sys, storage_timeseries, hour)
    # Run simulation for all days
    interval = 24
    horizon = 24
    steps = 1
    day_list = collect(239:365)
